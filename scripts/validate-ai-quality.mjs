...
for (const topic of topics) {
  ...
  const evidenceFailure = validateAIEvidenceClaims({ summary, why_now: whyNow, risks: topic.ai_risks, opportunities }, topic, topic.raw_signals || []);
  if (evidenceFailure) reasons.push(evidenceFailure);
...
if (!/topic\.raw_signals/.test(aiEnricher) || !/latest_captured_at/.test(aiEnricher) || !/validateAIEvidenceClaims/.test(aiEnricher) || !/AI source snapshot is stale or invalid/.test(aiEnricher)) {
