...
const sourceUrl = process.env.AI_SOURCE_URL || 'https://internet-trend-radar.linyuxuanlin.workers.dev/api/dashboard';
...
function validAI(topic, now) {
  ...
  if (!String(topic.ai_summary || '').trim()) return false;
  if (!String(topic.ai_why_now || '').trim()) return false;
...
if (dashboard.preview !== false || dashboard.ready !== true || !Array.isArray(dashboard.topics) || !dashboard.topics.length) {
  throw new Error('AI enrichment requires a real-data ready dashboard snapshot');
}
...
if (source) {
  ...
  dashboard.topics = dashboard.topics.map(topic => {
    ...
    return {
      ...
      ai_why_now: ai.ai_why_now,
      ai_risks: ai.ai_risks || '',
      ai_updated_at: ai.ai_updated_at,
      opportunities: ai.opportunities.slice(0, 3),
      ai_provenance: {
        provider: 'cloudflare-workers-ai',
        source: 'worker-dashboard',
        verified_non_heuristic: true,
        source_generated_at: source.generatedAt || null
      }
    };
  });
}
...
console.log(`AI overlay: matched=${matchedCount}, candidates=${candidateCount}, sourceReady=${Boolean(source)}${fetchError ? `, error=${fetchError}` : ''}`);

if (matchedCount < minMatches) {
  throw new Error(`AI overlay matched ${matchedCount} topics; require at least ${minMatches}`);
}
