...
if (!html.includes("https://radar.wiki-power.com/api/topic/")) {
  ...
  $('#dSourceSummary').innerHTML=(t.raw_signals||[]).map(s=>{const heatPath=s.metric_paths?.heat||s.peak_evidence?.heat?.metric_path||null;const engagementPath=s.metric_paths?.engagement||s.peak_evidence?.engagement?.metric_path||null;return \`<div><b>${'${esc(s.source_name||s.source_id)}'}</b> · 类型 ${'${esc(s.source_kind||\'未声明\')}'} · 榜单名次 ${'${esc(s.best_rank||\'-\')}' } · 原始 heat ${'${esc(metricValue(s.raw_heat_max))}'} · engagement ${'${esc(metricValue(s.raw_engagement_max))}'}<br><span class="summary">实际字段：${'${esc(heatPath||\'NULL\')}'} / ${'${esc(engagementPath||\'NULL\')}'}<br>字段定义：${'${esc(s.metric_definition?.heat||\'未声明\')}'} / ${'${esc(s.metric_definition?.engagement||\'未声明\')}'}<br>upstream：${'${esc(s.upstream||\'未记录\')}'} · 采集时间：${'${esc(s.latest_captured_at||\'未记录\')}'}</span></div>\`}).join('')||'原始指标暂无';
