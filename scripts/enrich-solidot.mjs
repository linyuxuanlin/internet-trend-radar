...
const FEED = 'https://www.solidot.org/index.rss';
...
async function fetchFeed(timeoutMs = 15000) {
  const response = await fetch(FEED, {
    redirect: 'follow',
    headers: {
      'user-agent': 'Mozilla/5.0 trend-radar/1.7',
...
function makeTopic(row, total, capturedAt) {
  ...
  return {
    ...
    category: categoryFor('solidot', row.title),
...
try {
  ...
  console.log(`OK solidot: ${topics.length}; dashboard topics=${dashboard.topics.length}`);
} catch (error) {
  ...
  await writeFile(DASHBOARD, JSON.stringify(dashboard, null, 2) + '\n', 'utf8');
  console.warn(`FAIL solidot: ${message}`);
  if (process.env.REQUIRE_SOLIDOT === '1') throw error;
}
