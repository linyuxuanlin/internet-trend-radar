...
function mergeTopics(existing, incoming) {
  ...
  for (const topic of incoming) {
    ...
    const seen = new Set((old.sources || []).map(source => source.source_id));
    old.sources = [...(old.sources || []), ...(topic.sources || []).filter(source => !seen.has(source.source_id))];
...
async function enrichOne(dashboard, sourceId, required) {
  ...
  try {
    ...
    console.log(`OK ${sourceId}: ${topics.length} real items via ${upstream || 'unknown upstream'}`);
    return true;
  } catch (error) {
...
console.log(JSON.stringify({ ok: true, required: REQUIRED, optional: OPTIONAL, topics: dashboard.topics.length, healthySources: healthy }));
